          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => handleVolumeSelect(e.target.value === 'all' ? 'all' : Number(e.target.value))}
                disabled={loading}
                className={clsx(
        ...
        <div className="bg-card border border-theme rounded-xl p-12 shadow-soft">
          <div className="flex items-center justify-center">
            ...
            <span className="ml-3 text-muted">
              Loading hadith from {selectedVolume === 'all' ? 'All Al-Kāfi Volumes' : `Volume ${selectedVolume}`}...
            </span>
          </div>
        </div>
      )}
      ...
      {!loading && !error && randomHadith && (
        <>
          <div className="flex items-center justify-center gap-2 text-sm text-slate-600 dark:text-slate-400">
            ...
            <span>
              Random hadith from {selectedVolume === 'all' ? 'All Al-Kāfi Volumes' : `Al-Kāfi Volume ${selectedVolume}`}
            </span>
          </div>
