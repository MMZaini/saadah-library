...
export default function GenericChapterDetailPage() {
  ...
  const [chapterInfo, setLocalChapterInfo] = useState<{ category: string; chapter: string; hadithCount: number } | null>(null)
  ...
  useEffect(() => {
    const loadChapter = async () => {
      ...
      try {
        ...
        const chapterHadiths = allHadiths.filter(h => h.categoryId === categoryId && h.chapterInCategoryId === chapterInCategoryId)
        ...
        const info = { category: first.category || 'Unknown', chapter: first.chapter || 'Unknown', hadithCount: chapterHadiths.length }
        setLocalChapterInfo(info)
        setChapterInfo({ volumeId: first.volume || 0, category: info.category, chapter: info.chapter, hadithCount: info.hadithCount })
  ...
  <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-6">
          {hadiths.map((h, idx) => (
            <div key={h._id || h.id || idx} className="relative">
              <div className="absolute -left-4 top-6 w-8 h-8 bg-accent-primary rounded-full flex items-center justify-center text-white text-sm font-bold shadow-medium">{idx + 1}</div>
              <div className="ml-8">
                <HadithCard hadith={h} />
              </div>
            </div>
        ...
        <div className="mt-12 pt-8 border-t border-theme">
          <button onClick={() => router.push('/')} className="inline-flex items-center gap-2 px-4 py-2 bg-card border border-theme rounded-lg hover:bg-hover-color text-primary">
