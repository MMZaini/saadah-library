...
;(() => {
  ...
  ;((a.id = 202),
    ...
    (a.modules = {
      ...
      18434: (a, b, c) => {
        ...
        async function y(a) {
          try {
            let b = a.nextUrl.searchParams,
              c = b.get('q'),
              d = b.get('book')
            if (!c) return u.NextResponse.json({ results: [], total: 0 })
            let e = d
              ? d
                  .split(',')
                  .map((a) => a.trim())
                  .filter(Boolean)
              : void 0
            if ((0, w.isArabicQuery)(c)) {
              let a = await (0, x.Z7)(c, e)
              return u.NextResponse.json(a)
            }
            if (e && e.length > 0) {
              let a = await (0, x.Lh)(c, e)
              return u.NextResponse.json(a)
            }
            let f = await v.C$.searchAllBooks(c)
            return u.NextResponse.json(f)
          } catch {
        ...
        let z = new e.AppRouteRouteModule({
            ...
            relativeProjectDir: '',
            resolvedPagePath:
              'C:\\Users\\Admin\\Desktop\\sl\\saadah-library\\app\\api\\search\\route.ts',
            nextConfigOutput: '',
            userland: d,
          }),
  ...
  b.C(a)
  var c = b.X(0, [331, 692, 999], () => b((b.s = 18434)))
  module.exports = c
})()
